            ...
            try
            {
                ...
                if (dialog == DialogResult.Yes)
                {
                   ...
                   if (dataReader.Read())
                    ...
                    /*   DialogResult dialog1 = MessageBox.Show("Uygulamaya Giriş Yapmak İstiyor Musunuz ?","SORU",MessageBoxButtons.YesNo,MessageBoxIcon.Question);
                     if (dialog1==DialogResult.Yes)
                     {
                         string mail, sifre; int id;

                         dataReader = sqlService.Reader("Select ID from KULLANICILAR where MAIL="+txtMail.Text);
                         if (dataReader.Read())
                         {
